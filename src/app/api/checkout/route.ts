...
interface LineItem {
 ...
 async function POST(req: Request) {
  try {
    const { lineItems }: { lineItems: LineItem[] } = await req.json();
    ...
    if (
      ...
      return NextResponse.json(
        { error: "LineItems inválidos", lineItems },
        { status: 400 }
      );
    }
    ...
    if (!domain || !token) {
      return NextResponse.json(
        { error: "Domínio ou token ausente" },
        { status: 500 }
      );
    }
    ...
    if (!checkoutUrl) {
      return NextResponse.json(
        {
          error: "Não foi possível gerar checkout",
