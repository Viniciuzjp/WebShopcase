...
function ProductCard({ produto, parsePrice, formatBRL }: { produto: ProductProps, parsePrice: (price: string) => {min: number, max: number}, formatBRL: (price: number) => string }) {
      ...
      <div className="flex max-md:flex-col md:flex-col lg:flex-col xl:flex-row gap-4">
        <section id="modal" className="flex flex-col px-5 xl:w-3/10 gap-10">
          <div className="flex mt-15 flex-col gap-3">
            ...
            <ul className="flex flex-col gap-5">
              {["MOSTRAR TODOS", "MAIOR PREÇO", "MENOR PREÇO"].map((label, index) => {
                const onClick = index === 0 ? handleFilterAll : index === 1 ? handleFilterMax : handleFilterMin
                return (
                  <li key={index} className="flex flex-col gap-5">
                    <div className="flex justify-between items-center border-b-2 border-neutral-200 p-2">
                      ...
                      <label className="relative cursor-pointer">
                        <input type="radio" onClick={onClick} className="peer absolute opacity-0 w-6 h-6" name="filter" />
        ...
        <section id="products" className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 sm:p-10 lg:grid-cols-2 xl:grid-cols-3 w-full gap-4">
          {categoria.map((produto) => (
            <ProductCard key={produto.pid} produto={produto} parsePrice={parsePrice} formatBRL={formatBRL} />
          ))}
        </section>
      </div>
    </main>
